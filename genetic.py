...
def Fitness(x,y,endX,endY,step):
    return int(Distance(x,y,endX,endY))# + step/2)
...
def TestPath(maze,stX,stY,length,endX,endY):
    ...
    breaked = 0
    for i in range(length):
        if len(AdjacentBrut_List(matrix,x,y,1)) != 0:
            break
        liste = AdjacentBrut_List(matrix,x,y,0)
        if len(liste) == 0:
            maze[x][y] = -1
            x,y = path[-1]
            path.pop()
    ...
    for i in range(len(path)):
        if len(AdjacentBrut_List(matrix,x,y,1)) != 0:
            ...
            if maze[nx,ny] != -1:
                ...
                step += 1
            matrix[x][y] = 3
    return int(Distance(x,y,endX,endY)) - step
...
def TestPathBackward(maze,path,stX,stY,endX,endY):
    ...
    return Fitness(x,y,endX,endY,step)
...
def Mutation(population,taux_muta):
    ...
    for i in range(muta):
        path = random.randint(0,len(population)-1)
        index = random.randint(0,len(population[path])-1)
        population[path][index] = random.randint(0,7)
    return population
